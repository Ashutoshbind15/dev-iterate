...
const triggerWebhook = async (payload: {
  ...
  const WORKFLOW_ID = "personalized_coding_question_generator";
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to trigger personalized coding question generation webhook: ${response.status} - ${errorText}`
    );
  }
};

export const triggerCodingQuestionGeneration = internalAction({
  args: {
    userId: v.id("users"),
