...
/**
 * Create a new personalized coding submission.
 ...
 */
export const createPersonalizedCodingSubmission = mutation({
  args: {
    questionId: v.id("personalizedCodingQuestions"),
    languageId: v.number(),
    sourceCode: v.string(),
  },
  returns: v.id("personalizedCodingSubmissions"),
  handler: async (ctx, args) => {
    ...
    const submissionId = await ctx.db.insert("personalizedCodingSubmissions", {
    ...
    await ctx.scheduler.runAfter(
      0, // Run immediately
      internal.actionsdir.personalizedCodeExecution.triggerPersonalizedCodeExecution,
      {
        submissionId,
        questionId: args.questionId,
        languageId: args.languageId,
...
/**
 * Update a personalized submission's status and results.
 * This is an INTERNAL mutation - only callable from trusted backend (judge integration).
 * Should NOT be exposed to clients.
 */
export const updatePersonalizedCodingSubmissionResult = internalMutation({
  args: {
...
/**
 * Mark a personalized submission as running.
 * Called when the judge starts processing the submission.
 */
export const markPersonalizedSubmissionRunning = internalMutation({
  args: {
    submissionId: v.id("personalizedCodingSubmissions"),
  },
  returns: v.null(),
