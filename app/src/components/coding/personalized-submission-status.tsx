...
import type { PersonalizedSubmission } from "@/hooks/use-personalized-coding-submission";

interface PersonalizedSubmissionStatusProps {
  submission: PersonalizedSubmission;
  compact?: boolean;
}

export function PersonalizedSubmissionStatus({
  submission,
  compact = false,
}: PersonalizedSubmissionStatusProps) {
  ...
  return (
    <div className="rounded-lg border border-slate-700/50 bg-slate-900/50 overflow-hidden">
      {/* Status Header */}
      <div
        className={`px-4 py-3 flex items-center gap-3 ${getStatusHeaderBg(
          status
        )}`}
      >
...
function getStatusHeaderBg(status: PersonalizedSubmission["status"]): string {
  switch (status) {
    case "queued":
...
function StatusIcon({
  ...
  status: PersonalizedSubmission["status"];
  size?: "sm" | "md";
}) {
...
function StatusLabel({ status }: { status: PersonalizedSubmission["status"] }) {
  switch (status) {
    case "queued":
      return "Queued";
    case "running":
...
interface FailureDetailsProps {
  failureIndex?: number;
  failure: NonNullable<PersonalizedSubmission["firstFailure"]>;
}
