...
import { usePersonalizedCodingSubmission } from "@/hooks/use-personalized-coding-submission";
import { PersonalizedSubmissionStatus } from "@/components/coding/personalized-submission-status";
import { PersonalizedSubmissionHistory } from "@/components/coding/personalized-submission-history";
import {
  JUDGE0_LANGUAGES,
  getLanguageName,
  getMonacoLanguageId,
...
export default function PersonalizedCodingSolvePage() {
  const { questionId: questionIdParam } = useParams<{ questionId: string }>();
  const navigate = useNavigate();
  const questionId = questionIdParam as
    | Id<"personalizedCodingQuestions">
    | undefined;
  ...
  const question = useQuery(
    api.queries.personalizedCodingQuestions
      .getPersonalizedCodingQuestionForSolve,
  ...
  const {
    ...
    trackSubmission,
    trackedSubmissionId,
  } = usePersonalizedCodingSubmission({
  ...
  const handleSelectHistoricalSubmission = (
    submissionId: Id<"personalizedCodingSubmissions">
  ) => {
    trackSubmission(submissionId);
  };
      ...
      <div className="min-h-screen bg-slate-950 p-8 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/personalized-coding">
            <Button variant="outline">Go to Personalized Coding</Button>
          </Link>
        </div>
      </div>
    );
          ...
          >
            ...
            {currentSubmission && (
              <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 p-4">
                ...
                <PersonalizedSubmissionStatus submission={currentSubmission} />
              </div>
            )}
